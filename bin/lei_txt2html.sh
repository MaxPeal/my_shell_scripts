...
sed \
    -e 's/>/\&gt\;/g' \
    -e 's/</\&lt\;/g' \
    -e '/^[Aa]rt\.\?[[:blank:]]*[1-9][[:blank:]]*[º°][[:blank:]]*\(\.\|\-\|\)[[:blank:]]*/,/^$/ {
        /^[Aa]rt\.\?[[:blank:]]*[1-9][[:blank:]]*[º°][[:blank:]]*\(\.\|\-\|\)[[:blank:]]*/ s/^[Aa]rt\.\?[[:blank:]]*\([1-9]\)[[:blank:]]*[º°][[:blank:]]*\(\.\|\-\|\)[[:blank:]]*/<p style="text-align: justify\;">\n<b>Art\. \1\&ordm\;\.<\/b> /
        /^$/ s/^/<\/p>/
    }' \
    -e '/^[Aa]rt\.\?[[:blank:]]*[1-9][0-9]\+[[:blank:]]*\(\.\|\-\|\)[[:blank:]]*/,/^$/ {
        /^[Aa]rt\.\?[[:blank:]]*[1-9][0-9]\+[[:blank:]]*\(\.\|\-\|\)[[:blank:]]*/ s/^[Aa]rt\.\?[[:blank:]]*\([1-9][0-9]\+\)[[:blank:]]*\(\.\|\-\|\)[[:blank:]]*/<p style="text-align: justify\;">\n<b>Art\. \1\.<\/b> /
        /^$/ s/^/<\/p>/
    }' \
    -e '/^[Pp]arágrafo[[:blank:]]*[Úú]nico\.\?[[:blank:]]*/,/^$/ {
        /^[Pp]arágrafo[[:blank:]]*[Úú]nico\.\?[[:blank:]]*/ s/^[Pp]arágrafo[[:blank:]]*[Úú]nico\.\?[[:blank:]]*/<p style="text-align: justify\;">\n<b>Par\&aacute\;grafo \&uacute\;nico\.<\/b> /
        /^$/ s/^/<\/p>/
    }' \
    -e '/^§[1-9][0-9]*[º°]\.\?[[:blank:]]*/,/^$/ {
        /^§[1-9][0-9]*[º°]\.\?[[:blank:]]*/ s/^§\([1-9][0-9]*\)[º°]\.\?[[:blank:]]*/<p style="text-align: justify\;">\n<b>\&sect\;\1\&ordm\;\.<\/b> /
        /^$/ s/^/<\/p>/
    }' \
    -e '/^CAPÍTULO/,/^$/ {
    ...
    -e 's/\([Ss]\)tricto[[:blank:]]\+\([Ss]\)ensu/<i>\1tricto \2ensu<\/i>/g' \
    -e 's/\([Ll]\)ato[[:blank:]]\+\([Ss]\)ensu/<i>\1ato \2ensu<\/i>/g' \
    -e 's/—/-/g' \
    -e 's/º/\&ordm\;/g' \
    -e 's/À/\&Agrave\;/g' \
    -e 's/à/\&agrave\;/g' \
